...
def bedrock_to_detritus(bedrock_collection: EventCollection) -> BankLedger:
    ...
    for event in bedrock_collection.events:
        ...
        if event.card_swipe:
            cs = event.card_swipe
            # TODO: This is a hack. Skipping events with no timestamp is not robust;
            # should validate bedrock events earlier and report errors instead of silently skipping.
            if not cs.timestamp:
    ...
    # TODO: Not yet handling RemovePending or UpdateBalance BankEvent types from the detritus proto.
    # Implement these as needed for full ledger support.
